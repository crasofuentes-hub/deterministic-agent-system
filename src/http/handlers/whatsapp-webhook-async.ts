...
export interface HandleAsyncWhatsAppWebhookOptions {
  verifyToken: string;
  bodyText?: string;
  loadSession?: (customerId: string) => SessionState | Promise<SessionState>;
  deliveryMode?: AsyncWhatsAppDeliveryMode;
...
function isValidMetaWebhookSignature(
  bodyText: string,
  appSecret: string,
...
export async function handleAsyncWhatsAppWebhook(
  ...
  options: HandleAsyncWhatsAppWebhookOptions
): Promise<void> {
  ...
  for (const message of normalized.value) {
    ...
    const session = options.store
      ? await options.store.loadSession(message.customerId)
      : (await options.loadSession?.(message.customerId)) ??
    ...
    let delivery:
      | {
          mode: AsyncWhatsAppDeliveryMode;
          result: unknown;
    ...
    if (options.store) {
      await options.store.saveConversationEvent({
      ...
      await options.store.saveSession(message.customerId, bridge.session);

      await options.store.saveEvidence({
        customerId: message.customerId,
      ...
      if (
        ...
        await options.store.saveHandoff({
          handoffId,
          customerId: message.customerId,
