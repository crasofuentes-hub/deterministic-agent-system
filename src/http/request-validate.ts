...
export function validateExecuteRequest(input: unknown): RequestValidationResult {
  ...
  if (mode !== "local" && mode !== "mock") {
    return { ok: false, error: "mode must be 'local' or 'mock'" };
  }
  ...
  if (typeof traceId !== "undefined") {
    ...
    if (traceId.length > 256) {
      return { ok: false, error: "traceId exceeds 256 characters" };
    }
  }

  const plan = input.plan;
  if (!isPlan(plan)) {
    return { ok: false, error: "plan must be a valid version 1 plan object" };
  }
