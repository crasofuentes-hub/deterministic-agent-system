...
test("runs: complete returns 409 when run is still CREATED (deterministic)", async () => {
  ...
  try {
    ...
    const created = await requestJson(base + "/runs", {
      ...
      body: JSON.stringify({ agentId: "agent-complete-created-001", input: { goal: "complete-without-start" } }),
    });
