...
test("runs: start returns 409 when run is cancelled", async () => {
  ...
  try {
    ...
    const created = await requestJson(base + "/runs", {
      ...
      body: JSON.stringify({
        agentId: "agent-start-cancelled-001",
        input: { goal: "cancel-then-start" }
      }),
    });
    ...
    // start should be rejected deterministically
    ...
    assert.equal(started.body.error.code, "INVALID_RUN_TRANSITION");
  } finally {
    await running.close();
  }
});
