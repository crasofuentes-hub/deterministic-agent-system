...
test("http POST /agent/run returns PLAN_EXCEEDS_MAX_STEPS deterministically", async () => {
  ...
  try {
    ...
    assert.equal(r1.body.error.code, "PLAN_EXCEEDS_MAX_STEPS");
  } finally {
    await running.close();
  }
});