...
describe("canonical-response-engine", () => {
  it("finds canonical response deterministically", () => {
    expect(
      ...
      stage: "collect-case-id",
      status: "missing-entity",
      messageTemplate: "Please provide your case ID so I can review the status.",
    });
  });
  ...
  it("renders canonical response text deterministically", () => {
    ...
    ).toBe("Please provide your case ID so I can review the status.");
  });
  ...
  it("requires canonical response and throws stable error when missing", () => {
    expect(() =>
      ...
      'CANONICAL_RESPONSE_NOT_FOUND: intentId="consult-status" stage="done" status="unknown-status"'
    );
  });
});