...
test("runs: start returns 400 INVALID_REQUEST when body is not an object", async () => {
  ...
  try {
    ...
    const created = await requestJson(base + "/runs", {
      ...
      body: JSON.stringify({ agentId: "agent-start-badbody-001" }),
    });
    assert.equal(created.status, 201);
    const runId = created.body.result.runId;

    const r = await requestJson(base + `/runs/${runId}/start`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify([]),
    });
